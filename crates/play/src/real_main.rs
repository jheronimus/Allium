...
use crate::core::{ActiveSession, PlayContext};
use crate::platform::{DefaultPlatform, init_logging};
use crate::video::frame_interval;
use crate::{dump, settings};
...
async fn start_main_loop(
    ...
    let command_server = tokio::spawn(async move {
        if let Err(err) = crate::commands::run_command_server(tx, state).await {
